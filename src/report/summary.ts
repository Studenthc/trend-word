...
export function summarizeRun(input: RunSummaryInput): RunSummary {
  ...
  const failedSources = input.sourceHealth.filter((item) => ["blocked", "unverified"].includes(item.status) || item.failureReasons.length > 0).map((item) => item.sourceType);
  ...
  const result: RunSummary = {
    ...
    sourcesSucceeded: input.sourceHealth.filter((item) => item.status === "available" || item.status === "partial").map((item) => item.sourceType),
    ...
    failedSources,
    partialSources,
    warningCount: failedSources.length + partialSources.length,
  };
