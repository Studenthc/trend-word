...
describe("candidate queue", () => {
  ...
  it.each(["AI scientist system builder", "internal tool builder"]) ("keeps concrete capability expressions in the formal pool: %s", (term) => {
    const demand: DemandExpression = {
    ...
    expect(result.formal.map((item) => item.term)).toContain(term);
  ...
  it("ranks direct feedback ahead of capability-derived demand at the same freshness", () => {
    ...
    const derived: DemandExpression = {
      ...direct, id: "demand-capability", text: "AI application builder", normalizedText: "ai application builder", rawSignalId: "capability-source", sourceEntityId: "entity-github-capability", sourceUrl: "https://github.com/acme/app-builder", evidenceQuote: "A platform for building AI applications.", qualityState: "review", qualityScore: 55, origin: "capability_derived", sourceText: "A platform for building AI applications.", transformation: "将 building + applications 改写为能力搜索词", evidencePrecision: "semantic",
    };
    const result = buildCandidateQueue([
      signal("feedback-source", { sourceType: "github", title: "acme/flowpilot#12", body: "I need an alternative to Zapier.", signalKind: "feedback", tags: ["feedback", "github-issue"] }),
      signal("capability-source", { sourceType: "github", title: "acme/app-builder", body: "A platform for building AI applications." }),
    ], { demandExpressions: [derived, direct], now: "2026-09-01T00:00:00.000Z" });
    expect(result.formal.map((item) => item.term)).toEqual(expect.arrayContaining(["replace Zapier", "AI application builder"]));
    expect(result.formal.findIndex((item) => item.term === "replace Zapier")).toBeLessThan(result.formal.findIndex((item) => item.term === "AI application builder"));
  });
