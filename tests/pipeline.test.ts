...
describe("runRadar", () => {
  it("persists model artifacts and keeps model-only demand in observation", async () => {
    ...
    const model = parseModelRecord({ id: "huggingface:acme/reference-to-video", platform: "huggingface", modelName: "acme/reference-to-video", modelUrl: "https://huggingface.co/acme/reference-to-video", updatedAt: "2026-09-02T00:00:00.000Z", inputTypes: ["image"], outputTypes: ["video"], claimedCapabilities: ["reference-to-video"], tags: ["reference-to-video"], notes: [], sourceSignalId: "model-signal", evidenceStatus: "verified" });
